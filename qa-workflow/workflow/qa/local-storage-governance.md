...
QA 任务的角色、双环数据流和证据要求见 `codex-qa-orchestration.md`；任务卡使用 `templates/qa-task-card.yaml`。测试轮次的长期结果统一归档到 `F:\[TARGET_APP]-Toolchain\results\<任务ID>`，临时运行数据归档到 `G:\[TARGET_APP]-Data\test`。
